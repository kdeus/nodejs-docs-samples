...
const text = `text`;
const lang = `lang`;
const savedFileName = `image_to_${lang}.txt`;
...
describe(`functions:ocr`, () => {
  ...
  it(`translateText fails without a lang`, () => {
    ...
    const event = {
      data: {
        data: Buffer.from(JSON.stringify({ text, filename: fileName })).toString(`base64`)
      }
    };
  ...
  it(`saveResult translates and publishes text`, () => {
    const event = {
      data: {
        data: Buffer.from(JSON.stringify({ text, filename: fileName, lang })).toString(`base64`)
      }
    };
    const sample = getSample();
    ...
    return sample.program.saveResult(event)
      .then(() => {
        ...
        assert.deepEqual(console.log.getCall(1).args, [`Saving result to ${savedFileName} in bucket ${sample.mocks.config.RESULT_BUCKET}`]);
